...
export function Clients({ initialClientId, initialOpenCreate }: { initialClientId?: string; initialOpenCreate?: boolean }) {
  ...
  const clientData = useMemo(() => {
    ...
    return {
      ...
      time: timeEntries.filter(t => t.clientId === cid)
    };
      ...
      <div className={`flex-col w-full md:w-[350px] lg:w-[400px] h-full bg-canvas shrink-0 border-r border-bd-subtle ${selectedClient ? 'hidden md:flex' : 'flex'}`}>
        <Header title="Clients" rightIcon={
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => { setForm(initialFormState); setEditingClientId(null); setIsSheetOpen(true); }} className="w-10 h-10 flex items-center justify-center rounded-full bg-tx-primary text-tx-inverse hover:opacity-90 transition-opacity">
              <Plus size={22} />
            </button>
          </div>
        } />
                    ...
                    clientData.revenues.map(r => (
                      <div key={r.id} className="bg-canvas border border-bd-subtle rounded-xl p-4 flex justify-between items-center">
                        <div>
                          ...
                          <div className="text-[13px] text-tx-muted">{new Date(r.date + 'T00:00:00').toLocaleDateString()}</div>
                        </div>
                        <div className={`px-2.5 py-1 rounded-full text-[11px] font-medium ${r.status === 'Paid' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-red-500/10 text-red-500'}`}>
                          {r.status}
                        </div>
                      </div>
                    ))
